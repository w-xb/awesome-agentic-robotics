...
A selective reading list of 100 influential works on autonomous physical agents, spanning planning, agentic VLA, memory, world models, verification, recovery, tool use, and safety.
...
Agentic robotics studies physical systems that maintain task state, reason over future consequences, invoke skills or tools, verify execution, recover from failures, adapt from experience, and operate under real-world safety constraints. The list includes both modular LLM/VLM-agent architectures and tightly coupled robot-learning systems when they make a clear contribution to closed-loop physical agency.

Entries are selected for relevance, influence, evidence quality, representativeness, and resource completeness. General-purpose VLA work is included only when it materially advances planning, long-horizon execution, tool use, verification, recovery, embodied memory, world-model-based decisions, continual adaptation, or cross-embodiment deployment. Each paper has one canonical entry, and items are ordered by first public release date within each section.
...
- \[2026.7] ST-WAM: Semantic-Temporal World Action Model for Robust Manipulation under Visual Distribution Shifts [paper](https://arxiv.org/abs/2607.28993) [project](https://thu-wangmx.github.io/st-wam/) [code](https://github.com/Thu-WangMX/ST-WAM-Semantic-Temporal-World-Action-Model) — Combines semantic and temporal world-action representations to maintain manipulation performance under visual changes and distribution shifts during long-horizon execution.
...
The 89 papers removed during the strict curation pass remain available in [extended-reading.md](extended-reading.md). They are useful adjacent work, but scored lower on direct agentic relevance, influence, evidence, representativeness, or resource completeness.
